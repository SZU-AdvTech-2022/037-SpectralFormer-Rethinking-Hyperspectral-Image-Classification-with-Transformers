...
parser.add_argument('--dataset', choices=['Indian', 'Pavia', 'Houston', 'YRE'], default='YRE', help='dataset to use')
parser.add_argument('--flag_test', choices=['test', 'train'], default='train', help='testing mark')
parser.add_argument('--mode', choices=['ViT', 'CAF'], default='CAF', help='mode choice')
parser.add_argument('--name', choices=['ViT', 'Pixel', 'Patch'], default='Patch', help='mode choice')
parser.add_argument('--gpu_id', default='5', help='gpu id')
parser.add_argument('--seed', type=int, default=121, help='number of seed')
...
def train_and_test_data(mirror_image, band, train_point, test_point, true_point, patch=5, band_patch=3):
    ...
    for j in range(test_point.shape[0]):
        ...
        # newX = np.reshape(newX, (-1, band)).transpose(1,0)
        # x = SpectralClustering(n_clusters=band//3).fit_predict(newX).transpose(1,0)
        # x_test[j, :, :, :] = np.reshape(x, (patch, patch, band//3))
    for k in range(true_point.shape[0]):
        x_true[k,:,:,:] = gain_neighborhood_pixel(mirror_image, true_point, k, patch)

    # x_train = np.reshape(x_train, (x_train.shape[0], patch * patch, band))
    # x_test = np.reshape(x_test, (x_test.shape[0], patch*patch, band))
    # x_true = np.reshape(x_true, (x_true.shape[0], patch * patch, band))
...
if args.dataset == 'Indian':
    ...
    input = h5py.File('./data/GF5.mat', 'r')['gf5']
    input = np.transpose(input, (2, 1, 0))
    data = loadmat('./data/label.mat')
else:
...
band = (band//4)*3
newX = np.reshape(mirror_image, (-1, mirror_image.shape[2]))
newX = PCA(n_components=band).fit_transform(newX)
mirror_image = np.reshape(newX, (mirror_image.shape[0], mirror_image.shape[1], band))
...
if args.flag_test == 'test':
    if args.name == 'ViT':
        model.load_state_dict(torch.load('./log/ViT_01000.pt'))
    elif (args.name == 'Pixel'):
        model.load_state_dict(torch.load('./log/Pixel_00500.pt'))
    elif (args.name == 'Patch'):
        # model_state_dict_save = {k: v for k, v in torch.load('./log/SpectralFormer_patch.pt').items()}
        # model_state_dict = model.state_dict()
        # model_state_dict.update(model_state_dict_save)
        model.load_state_dict(torch.load('./log/spectral/Patch_Pavia_00500.pt'))
    else:
    ...
    plt.margins(0, 0)
    #plt.savefig('/home2/mzc/SpectralFormer/'+args.dataset+'_'+ args.name+'_300.jpg', dpi=300)
    #plt.savefig('/home2/mzc/SpectralFormer/' + args.dataset + '_300.jpg', dpi=300)
    plt.savefig('/home2/mzc/SpectralFormer/pca/' + args.dataset + '_' + args.name + '.jpg',dpi=300)
    plt.show()
    ...
    for epoch in range(args.epoches): 
        ...
        # if ((epoch+1) % args.epoches == 0):
            ...
            # model_name = './log/spectral'.strip('/') + '/' + args.name + '_' + args.dataset + '_' + str(epoch + 1).zfill(5) + '.pt'
